...
balti_total_emession_by_year<-NEI %>% filter(fips == "24510") %>% group_by(year) %>% summarise(total_pm25=sum(Emissions))

#plot
#set up graphics device and plot
png(file = "plot2.png")
plot(balti_total_emession_by_year,
     xlab="Year",
     ylab="Total Emisions for Baltimore",
     main="Year Over Year Trend of Total Emissions in Baltimore",
     col="green",
     lwd=3,
     type="l")
trend<-lm(balti_total_emession_by_year$total_pm25~balti_total_emession_by_year$year)
abline(trend)
